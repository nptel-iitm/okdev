...
description: Loops through multiple GitLab issues and runs /replicate-and-kickoff sequentially against each one — fully replicate-then-fix one issue before moving to the next.
---

# Replicate and Kickoff Multi — Skill

You are a coordinator. For a list of GitLab issues, you run `/replicate-and-kickoff`
...
2. Spawn a **fresh top-level sub-agent via the Agent tool** for this single issue.
   - The sub-agent's prompt instructs it to execute the `/replicate-and-kickoff`
...
- **One fresh Agent sub-agent per issue.** Do NOT invoke `/replicate-and-kickoff`
